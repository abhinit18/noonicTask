 ...
 */
App.controller('enterpriseShipmentsController', function($scope, $http, baseurl, $cookies, $cookieStore, $stateParams,
    MY_CONSTANT, $timeout, $window, $state, ngDialog, convertdatetime) {
    'use strict';


    $scope.registerSubAdmin = function() {
        if ($scope.name == "" || $scope.userName == "" || $scope.password == "" || typeof($scope.name) == 'undefined' || typeof($scope.userName) == 'undefined' || typeof($scope.password) == 'undefined') {
            ...
            $http({
                method: 'POST',
                url: baseurl.url + '/register_sub_admin?access_token=' + $cookieStore.get("obj").accesstoken,
                data: {
                ...
                if (response.data.statusCode == 200) {
                    $scope.successMsg = "Sub-Admin successfully registered";
                ...
                $scope.errorMsg       = "Could not register Sub-Admin";
