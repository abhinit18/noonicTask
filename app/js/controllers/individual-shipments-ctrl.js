 ...
 */
App.controller('individualShipmentsController', function($scope, $http, baseurl, $cookies, $cookieStore, $stateParams,
...
$scope.slide = 0;
    ...
    $scope.getShipments = function(t){

if(t=="upcoming"){


$http.post(baseurl.url + '/individual_upcoming_shipment',
...
if(response.statusCode=="200"){
       ...
       var shipment = response.data;
                  ...
                  angular.forEach(shipment,function(column) {
                    var d = {};

                    console.log(column);

                    d._id = column._id;
                    d.shipper_name = column.shipper_name;
                    d.mobile = column.mobile;
                    d.estimated_fare = column.estimated_fare;
                    d.schedule = moment(column.start_date).format("DD/MM/YYYY h:m A");
...
$scope.getShipments("upcoming");
    ...
    $http.get(baseurl.url + 'api/admin/getSubscriptionInfo/' + $cookieStore.get('obj').accesstoken)
        .success(function(response, status) {
            if (status == 200) {
                var dataArray = [];
